        ...
        switch (_context3.prev = _context3.next) {
          ...
          case 10:
            ...
            if (!index[lookup[j].join(",")]) {
              lookup.push.apply(lookup, _toConsumableArray(fv.genSubdividedLookup.apply(fv, _toConsumableArray(lookup[j]))));
          ...
          case 28:
            if (!index.hasOwnProperty("".concat(lookup[i][0], ",").concat(lookup[i][1]))) {
              // mark as done
              index["".concat(lookup[i][0], ",").concat(lookup[i][1])] = true; // move old lookup to cleanLookup
